...
# Functions and parameters about running variable lattice Monte-Carlo (bulk case)
...
from vlmc_bulk import VLMC_bulk
...
def run_MC(init_stru):
    ...
    ecf_init = FrechetCellFilter(init_stru)
    init_stru_relax = BFGS(ecf_init)
    ...
    operations = [
        ...
        (prob_list[4] / prob_total, analyzer.safe_group_rattle),
        (prob_list[5] / prob_total, analyzer.slide_atoms)
    ]
    ...
    for step in range(max_iterations):
        ...
        ecf_cur = FrechetCellFilter(cur_stru)
        local_relax = BFGS(ecf_cur, maxstep=1.2)
...
if __name__ == "__main__":
    ...
    prob_grp_rtt = config['prob_grp_rtt']
    prob_sld = config['prob_sld']
    prob_list = [prob_mig_C, prob_add_C, prob_rmv_C, prob_rtt, prob_grp_rtt, prob_sld]
